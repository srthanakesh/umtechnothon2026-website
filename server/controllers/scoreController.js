...
const getScores = async (req, res) => {
  try {
    const { deliverable_id } = req.query;
    
    if (!deliverable_id) {
      return res.status(400).json({ error: 'Deliverable ID is required' });
    }
    
    const scores = await scoreModel.getScoresByDeliverableId(deliverable_id);
    const totalScore = await scoreModel.getTotalScoreForDeliverable(deliverable_id);
...
const getLeaderboard = async (req, res) => {
  try {
    ...
    const leaderboard = teamScores
      .map(score => ({
        team_name: teamMap[score.team_id],
